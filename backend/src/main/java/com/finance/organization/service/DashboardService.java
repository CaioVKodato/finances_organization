...
public class DashboardService {
    ...
    public DashboardSummary summary() {
        ...
        for (SpentBy sb : Arrays.asList(SpentBy.values())) {
            bySpentBy.put(sb.name(), expenseRepository.sumAmountBySpentBy(sb));
        }

        LocalDate today = LocalDate.now();
        LocalDate monthStart = today.withDayOfMonth(1);
        LocalDate monthEnd = today.withDayOfMonth(today.lengthOfMonth());
        BigDecimal spentAllMonth = expenseRepository.sumAmountBetween(monthStart, monthEnd);
        BigDecimal spentSelfMonth = expenseRepository.sumAmountBetweenAndSpentBy(
                monthStart, monthEnd, SpentBy.SELF
        );
        BigDecimal income = settingsService.getMonthlyIncomeOrZero();
